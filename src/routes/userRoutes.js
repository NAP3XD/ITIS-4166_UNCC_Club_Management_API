...
// Get user by ID
router.get('/:id', validateUserId, authenticate, authorizeRoles('API_ADMIN'), getUserByIdHandler);

// Create new user (Admin only)
router.post('/', validateCreateUser, authenticate, authorizeRoles('API_ADMIN'), createUserHandler);

// Update user by ID
router.put('/:id', validateUserId, validateUpdateUser, authenticate, authorizeRoles('API_ADMIN'), updateUserHandler);
